...
const createCollege = async (req, res) => {
  try {
    ...
    if (!name) return res.status(404).send({ status: false, msg: "Please Provide Name" });

    if (!fullName) return res.status(404).send({ status: false, msg: "Please Provide FullName" });

    if (!validator.isValidName(name)) return res.status(404).send({ status: false, msg: "Please Provide a Valid Name" });

    if (!validator.isValidFullName(fullName)) return res.status(404).send({ status: false, msg: "Please Provide a Valid Full Name" });
